"""
Direct SQLite update for ELE304 BOA articles (bypasses SQLAlchemy).
"""
import sqlite3
import json
import os
...
boa = {
    ...
    "article_6": "1. Cualificaciones profesionales completas:\na) Gestión y supervisión del montaje y mantenimiento de las infraestructuras de telecomunicación y de redes de voz y datos en el entorno de edificios ELE383_3 (Real Decreto 328/2008, de 29 de febrero): UC1184_3, UC1185_3, UC1186_3, UC1187_3.\nb) Gestión y supervisión del montaje y mantenimiento de sistemas de producción audiovisual y de radiodifusión ELE487_3 (Real Decreto 144/2011, de 4 de febrero): UC1578_3, UC1579_3, UC1580_3, UC1581_3.\nc) Desarrollo de proyectos de infraestructuras de telecomunicación y de redes de voz y datos en el entorno de edificios ELE258_3 (Real Decreto 1115/2007, de 24 de agosto): UC0826_3, UC0827_3, UC0828_3.",
    "article_6_cps": [
    ...
    "article_9_og": [
        ...
        {"id": "b", "desc": "Reconocer sistemas de telecomunicaciones, aplicando leyes y teorías fundamentales, para calcular los parâmetros de equipos, elementos e instalaciones."},
...
# Get existing data to preserve article_9_og if already there
cur.execute('SELECT boa_articles FROM degrees WHERE code = ?', ('ELE304',))
row = cur.fetchone()
existing = json.loads(row[0]) if row and row[0] else {}

# Merge: keep existing article_9_og from previous script run
if 'article_9_og' in existing and len(existing['article_9_og']) > 20:
    boa['article_9_og'] = existing['article_9_og']
    print(f"Preserved existing article_9_og ({len(existing['article_9_og'])} items)")
